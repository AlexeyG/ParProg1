...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    if (left.isInstanceOf[Empty] && right.isInstanceOf[Empty])
      ...
      elem.moreRetweets(right.mostRetweeted)
    else if (right.isInstanceOf[Empty])
      elem.moreRetweets(left.mostRetweeted)
    else
      elem.moreRetweets(right.mostRetweeted).moreRetweets(left.mostRetweeted)
  }
...
object GoogleVsApple {
  ...
  def hasWords(words: List[String]): Tweet => Boolean = tw => words.exists(tw.text.contains)

  lazy val googleTweets: TweetSet = allTweets.filter(hasWords(google))
  lazy val appleTweets: TweetSet = allTweets.filter(hasWords(apple))
